//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.2.6 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2014.09.25 at 01:41:27 PM EDT 
//


package org.mitre.cybox.objects;

import java.util.ArrayList;
import java.util.List;
import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlType;


/**
 * VolumeType characterizes the volume information in the Windows prefetch file.
 * 
 * <p>Java class for VolumeType complex type.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * 
 * <pre>
 * &lt;complexType name="VolumeType">
 *   &lt;complexContent>
 *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *       &lt;sequence>
 *         &lt;element name="VolumeItem" type="{http://cybox.mitre.org/objects#WinVolumeObject-2}WindowsVolumeObjectType" maxOccurs="unbounded" minOccurs="0"/>
 *         &lt;element name="DeviceItem" type="{http://cybox.mitre.org/objects#DeviceObject-2}DeviceObjectType" maxOccurs="unbounded" minOccurs="0"/>
 *       &lt;/sequence>
 *     &lt;/restriction>
 *   &lt;/complexContent>
 * &lt;/complexType>
 * </pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "VolumeType", namespace = "http://cybox.mitre.org/objects#WinPrefetchObject-2", propOrder = {
    "volumeItems",
    "deviceItems"
})
public class VolumeType {

    @XmlElement(name = "VolumeItem")
    protected List<WindowsVolume> volumeItems;
    @XmlElement(name = "DeviceItem")
    protected List<Device> deviceItems;

    /**
     * Gets the value of the volumeItems property.
     * 
     * <p>
     * This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the volumeItems property.
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * <pre>
     *    getVolumeItems().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link WindowsVolume }
     * 
     * 
     */
    public List<WindowsVolume> getVolumeItems() {
        if (volumeItems == null) {
            volumeItems = new ArrayList<WindowsVolume>();
        }
        return this.volumeItems;
    }

    /**
     * Gets the value of the deviceItems property.
     * 
     * <p>
     * This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the deviceItems property.
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * <pre>
     *    getDeviceItems().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link Device }
     * 
     * 
     */
    public List<Device> getDeviceItems() {
        if (deviceItems == null) {
            deviceItems = new ArrayList<Device>();
        }
        return this.deviceItems;
    }

}
